...
class Listify_Results {
	...
	public static function register_scripts() {
		...
		$ver   = $debug ? time() : listify_get_version();
		$min   = ! $debug ? '.min' : '';
		...
		// Listings.
		wp_register_script( 'listify-listings', $js . "/listings{$min}.js", array( 'listify-results', 'jquery', 'wp-util' ), $ver, true );
		...
		wp_register_script( 'listify-googlemaps', $js . "/map-googlemaps{$min}.js", $deps, $ver, true );

		// Google Maps.
		wp_register_script( 'google-maps', listify_get_google_maps_api_url(), array(), '3.31', true );

		// Infobubble.
		wp_register_script( 'infobubble', $js . '/vendor/googlemaps/infobubble/infobubble.js', array( 'google-maps' ), '0.8', true );

		// Markerclusterer.
		wp_register_script( 'markerclusterer', $js . '/vendor/googlemaps/markerclusterer/markerclusterer.js', array( 'google-maps' ), '1.2.0', true );

		// Richmarker.
		wp_register_script( 'richmarker', $js . '/vendor/googlemaps/richmarker/richmarker.js', array( 'google-maps' ), $ver, true );
		...
		// Leaflet.
		wp_register_script( 'leaflet', '//unpkg.com/leaflet@1.0.3/dist/leaflet.js', array(), '1.0.3', true );
		wp_register_style( 'leaflet', '//unpkg.com/leaflet@1.0.3/dist/leaflet.css', array(), '1.0.3' );

		// Leaflet Markercluster.
		wp_register_script( 'leaflet-markercluster', $js . '/vendor/mapbox/markerclusterer/leaflet.markercluster.js', array( 'leaflet' ), $ver, true );

		// Leaflet Geosearch.
		wp_register_script( 'leaflet-geosearch', $js . '/vendor/mapbox/geosearch/leaflet.geosearch.js', array( 'leaflet' ), $ver, true );

		// Columns.
		$columns = absint( get_theme_mod( 'listing-archive-display-columns', 2 ) );

		// Deprecated column class filter. Added for back compat.
		// Should use "listify_get_listings_args" filter to override column.
		...
		$settings = array(
			...
			'mapService'        => array(
				...
				'autofit'     => (bool) get_theme_mod( 'map-behavior-autofit', true ),
				'autoPan'     => get_theme_mod( 'map-behavior-autopan', true ) && ! wp_is_mobile() ? 1 : 0,
			...
			'i18n'              => array(
				...
				'resultsShowing' => sprintf( __( '(Showing %1$d-%2$d)', 'listify' ), 1, get_option( 'job_manager_per_page' ) ),
			),
