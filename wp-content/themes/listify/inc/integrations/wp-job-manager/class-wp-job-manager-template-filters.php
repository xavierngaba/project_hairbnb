...
class Listify_WP_Job_Manager_Template_Filters {
	...
	public static function enqueue_scripts( $atts = array() ) {
		...
		wp_enqueue_script( 'wnumb', $js . '/wNumb/wNumb.js', array(), listify_get_version(), true );
		wp_enqueue_script( 'nouislider', $js . '/nouislider/nouislider.js', array(), '9.2.0', true );
		wp_enqueue_style( 'nouislider', $js . '/nouislider/nouislider.css', array(), '9.2.0' );
	...
	public static function get_all_filters( $atts ) {
		return apply_filters(
			'listify_wp_job_manager_filters', array(
...
<?php
	...
	public static function get_category_filter( $atts = array() ) {
		...
		$dropdown_args = apply_filters(
			'listify_wp_job_manager_filters_dropdown_category', array(
...
if ( $atts['show_category_multiselect'] ) :
?>
 search_categories--multiselect<?php endif; ?>">
...
<?php
	...
	public static function get_sort_filter( $atts = array() ) {
		...
		// Selected.
		$selected = get_theme_mod( 'listing-filters-sort-default', 0 );
...
<div class="search-sort">
	...
	<select id="search_sort" class="job-manager-filter" name="search_sort" autocomplete="off">
		<?php // Default sort need to use empty value, to make sure it did not go blank in form reset. ?>
		<option value="" <?php selected( 0, $selected ); ?>><?php esc_html_e( 'Sort by', 'listify' ); ?></option>
		<?php foreach ( $options as $id => $option ) : ?>
			<option value="<?php echo esc_attr( $id ); ?>" <?php selected( $id, $selected ); ?>><?php echo esc_html( $option ); ?></option>
		<?php endforeach; ?>
	</select>
</div>
