...
class Listify_WP_Job_Manager_Map extends listify_Integration {
	...
	public function get_listings( $result, $listings ) {
		...
		if ( $listings->have_posts() ) {
			...
			$end      = $start + $listings->post_count - 1;

			if ( $end > $result['found_posts'] ) {
				$end = $result['found_posts'];
			}

			// Translators: %1$d starting number. %2$d Ending number.
			$result['offset'] = sprintf( __( '(Showing %1$d-%2$d)', 'listify' ), $start, $end );
		}
	...
	public function apply_proximity_filter( $query_args, $args ) {
		...
		$post_ids = self::geolocation_search(
			array(
				'latitude'  => $lat,
				'longitude' => $lng,
				'radius'    => $radius,
