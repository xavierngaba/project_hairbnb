...
class Listify_Widget_Listing_Related_Listings extends Listify_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $location && $listing->get_lat() && $listing->get_lng() ) {
			$related_location = Listify_WP_Job_Manager_Map::geolocation_search(
				array(
			...
			if ( ! empty( $related_location ) ) {
				...
				if ( empty( $args['post__in'] ) ) {
					$args['post__in'] = array( -99999 );
				}
			}
		}

		if ( $category ) {
			$args['search_categories'] = wp_get_post_terms(
				get_post()->ID, 'job_listing_category', array(
					'fields' => 'ids',
				)
			);
		}
		...
		$listings = listify_get_listings(
			array(
				...
				'query_args' => apply_filters( 'listify_widget_related_listings_args', $args ),
				'columns'    => '2',
			)
		);
