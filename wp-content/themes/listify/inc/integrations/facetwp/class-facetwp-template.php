...
class Listify_FacetWP_Template extends Listify_FacetWP {
	...
	public function __construct() {
		...
		// Update default result count text.
...
<div class="facets job_filters job_filters--<?php echo get_theme_mod( 'listing-filters-style', 'content-box' ); ?> <?php
if ( ! $front ) :
?>
content-box<?php endif; ?> <?php echo esc_attr( $this->position() ); ?>">
	<?php echo $this->output_facet_html( $default_facets ); // WPCS: XSS ok. ?>
	...
	<?php if ( ! empty( $more_facets ) ) : ?>
		<div class="more-filters">
			...
			<div class="more-filters__filters">
				<?php echo $this->output_facet_html( $more_facets ); // WPCS: XSS ok. ?>
			</div>
		</div>
	<?php endif; ?>
...
<?php
	 ...
	 * @since 2.5.0
	 *
	 * @param string $output Count to output.
	...
	public function facetwp_result_count( $output, $params ) {
		if ( $params['lower'] === $params['upper'] || 1 === $params['total'] ) {
			...
			return sprintf( _n( 'Showing the single result', 'Showing all %d results', $params['total'], 'listify' ), $params['total'] );
		}

		return sprintf(
			// translators: %1$s total count. %2$s lower count. %3$s upper count.
			__( '%1$s Results Found (Showing %2$s-%3$s)', 'listify' ),
			$params['total'],
			$params['lower'],
			$params['upper']
		);
