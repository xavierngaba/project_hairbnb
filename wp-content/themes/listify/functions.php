...
function listify_setup() {
	...
	add_theme_support(
		'custom-background', apply_filters(
			'listify_custom_background_args', array(
	...
	add_theme_support(
		'custom-header', apply_filters(
			'listify_custom_header_args', array(
				'video'              => true,
				'default-image'      => '',
...
function listify_scripts() {
	...
	if ( false !== $google ) {
		wp_enqueue_style( 'listify-fonts', esc_url( $google ), array(), listify_get_version() );
	}

	// Enqueue primary CSS and RTL.
	wp_enqueue_style( 'listify', get_template_directory_uri() . '/css/style.min.css', array(), listify_get_version() );
	...
	wp_enqueue_script( 'listify', get_template_directory_uri() . '/js/app.min.js', $deps, listify_get_version(), true );
	wp_enqueue_script( 'salvattore', get_template_directory_uri() . '/js/vendor/salvattore/salvattore.min.js', array(), listify_get_version(), true );

	// Polyfill for Flexbox.
	wp_enqueue_script( 'flexibility', get_template_directory_uri() . '/js/vendor/flexibility/flexibility.min.js', array(), listify_get_version(), true );
	wp_script_add_data( 'flexibility', 'conditional', 'lt IE 11' );

	wp_localize_script(
		'listify', 'listifySettings', apply_filters(
			'listify_js_settings', array(
...
function listify_get_cover_from_group( $args ) {
	...
	if ( ( ! isset( $args['images'] ) || empty( $args['images'] ) ) && ( isset( $args['object_ids'] ) && ! empty( $args['object_ids'] ) ) ) {
		...
		if ( ! $image ) {
			// We only want published objects (get_objects_in_term does not filter this).
			$objects = get_posts(
				array(
					'post__in'    => $args['object_ids'],
					'post_status' => 'publish',
			...
			$attachments = new WP_Query(
				array(
					'post_parent__in'        => ! empty( $objects ) ? $objects : array( -1 ),
			...
			if ( $attachments->have_posts() ) {
				...
				if ( $image && $image[0] !== $attachments->posts[0]->_company_avatar ) {
...
function listify_count_posts( $post_type, $user_id ) {
	...
	if ( false === $count ) {
		...
		$count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $wpdb->posts WHERE post_author = '%s' AND post_type = '%s' and post_status = 'publish'", $user_id, $post_type ) ); 
		// @codingStandardsIgnoreEnd

		set_transient( $post_type . $user_id, $count, 24 * HOUR_IN_SECONDS );
	}
...
$includes = array(
	...
	'listing/class-listing-embed.php',
...
$integrations = apply_filters(
	'listify_integrations', array(
		...
		'bb-plugin'                        => class_exists( 'FLBuilder' ), // Beaver Builder.
	)
);
